...
export default {
  ...
  methods: {
    ...
    async getAllGroups() {
      try {
        const got = await Rbac.getAllGroups()
        this.loading = true
    ...
    async confirmEdit() {
      ...
      if (this.cacheForm.name !== this.form.name || this.cacheForm.info !== this.form.info) {
        // eslint-disable-line
        const res = await Rbac.updateOneGroup(this.form.name, this.form.info, this.id)
    ...
    goToGroupEditPage(val) {
      ...
      this.id = selectedData.id
      this.$router.push({ path: '/rbac/role/edit', query: { id: selectedData.id } })
    },
    handleDelete(val) {
      let res
      this.$confirm('此操作将永久删除该分组, 是否继续?', '提示', {
        ...
        try {
          this.loading = true
          res = await Rbac.deleteOneGroup(val.row.id)
        } catch (e) {
          this.loading = false
          console.log(e)
        }
